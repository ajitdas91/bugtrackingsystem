...
<%String logid=(String)session.getAttribute("logid");if(logid==null){ %><jsp:forward page="index.jsp"/><%} %>
   <div class="wrapper">
	      ...
	      <%
			...
			PreparedStatement stmt1=con.prepareStatement("select errordetails,solution,prodid from solutions where expertid=?");
			stmt1.setString(1,loginid);
			ResultSet rs1=stmt1.executeQuery();
			boolean status=rs1.next();
			if(status)
			{
				PreparedStatement stmt=con.prepareStatement("select errordetails,solution,prodid from solutions where expertid=?");
				stmt.setString(1,loginid);
				ResultSet rs=stmt.executeQuery();
				out.println("<table class='table1' border='#'>");
				out.print("<tr><th>ErrorDetails</th><th>Solutions</th><th>Prodid</th><tr> ");
				while(rs.next())
				{
				out.print("<tr><td>"+rs.getString(1)+"</td>");
				out.print("<td>"+rs.getString(2)+"</td>");
				out.print("<td>"+rs.getString(3)+"</td><td><a href='solvebug.jsp?errordetails="+rs.getString(1)+"'>Solve</a></td></tr>");
				}
				out.print("</table>");
				}
			else
